...
RANDOM_STATE = 42
DATA_PATH = "Backend/AI/diabetes/diabetes_train.csv"
...
# Convert Education and Income to numeric
...
ord_feats = ["Age_ord", "GenHlth_ord", "Education", "Income"]
bin_feats = [
...
# Standard scaling
num_for_scaler = cont_feats + ["Age_ord", "GenHlth_ord", "Education", "Income", "BMI_x_Age"]
...
def preprocess_patient(patient_df):
    ...
    for col in ["Education", "Income"]:
        if col in p.columns and p[col].dtype == object:
            p[col] = pd.to_numeric(p[col], errors="coerce").fillna(df[col].mode()[0]).astype(int)
    # Derived features
