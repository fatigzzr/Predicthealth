...
APP_PORT = int(os.getenv("USERS_PORT", "8002"))

app = FastAPI(title="User Ingestion Service", version="0.1.0")
app.add_middleware(
    CORSMiddleware,
...
async def upload(user_id: str, req: Request):
    _require_bearer(req)
    body = (await req.body()).decode()
    _ = xmltodict.parse(body) if body else {}
    return _xml({"status": "accepted", "userId": user_id}, "IngestResponse")
